...
class MgnTrainer(object):
    def __init__(self, model, criterions):
        super(MgnTrainer, self).__init__()
        self.model = model
        self.criterions = criterions

    def train(self, epoch, train_loader, optimizer, print_freq = 10, logger = None):
        self.model.train()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        ...
        for i, inputs in enumerate(train_loader):
            ...
            input_images, pids = self._parse_data(inputs)

            t_loss, c_loss, acc = self._forward(input_images, pids)
            total_loss = t_loss +c_loss
            losses_meter.update(total_loss.item(), inputs[0].size(0))
            loss_t_meter.update(t_loss.item(), input_images.size(0))
            loss_c_meter.update(c_loss.item(), input_images.size(0))
            precisions.update(acc, 1)
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
            ...
            if (i + 1) % print_freq == 0:
                logger.info('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i + 1, len(train_loader),
                              ...
                              loss_t_meter.val, loss_t_meter.avg,
                              loss_c_meter.val, loss_c_meter.avg,
                              precisions.val, precisions.avg))
    ...
    def _forward(self, inputs, targets):
        outputs = self.model(inputs)
        t_loss = [self.criterions[0](output, targets) for output in outputs[1:4]]
        ce_loss = [self.criterions[1](output, targets) for output in outputs[4:]]
        acc = (outputs[4].max(1)[1] == targets).float().mean()
        t_loss = sum(t_loss) / len(t_loss)
        ce_loss = sum(ce_loss) / len(ce_loss)
        return t_loss, ce_loss, acc



class DualMgnTrainer(object):
    def __init__(self, model, criterions):
        super(DualMgnTrainer, self).__init__()
        self.model = model
        self.criterions = criterions
    ...
    def train(self, epoch, train_loader, optimizer, print_freq = 10, logger = None):
        ...
        for i, inputs in enumerate(train_loader):
            ...
            t_loss, c_loss, acc = self._forward(input_images, pids)
            total_loss = 0.1*t_loss +10*c_loss
            losses_meter.update(total_loss.item(), inputs[0].size(0))
            loss_t_meter.update(t_loss.item(), input_images.size(0))
            loss_c_meter.update(c_loss.item(), input_images.size(0))
            precisions.update(acc, 1)
    ...
    def _forward(self, inputs, targets):
        outputs = self.model(inputs)
        t_loss = [self.criterions[0](output, targets) for output in outputs[1:6]]
        ce_loss = [self.criterions[1](output, targets) for output in outputs[6:]]
        acc = (outputs[6].max(1)[1] == targets).float().mean()
        t_loss = sum(t_loss) / len(t_loss)
        ce_loss = sum(ce_loss) / len(ce_loss)
        return t_loss, ce_loss, acc


class PartTrainer(object):
    def __init__(self, model, criterions):
        super(PartTrainer,self).__init__()
        self.model = model
        self.criterions = criterions
    ...
    def _forward(self,inputs, targets):
        outputs = self.model(inputs)
        ce_loss = self.criterions[1](outputs[1],targets)
        acc = (outputs[1].max(1)[1] == targets).float().mean()
        if isinstance(outputs[0],list):
            t_loss = 0.0
            for i in range(len(outputs[0])):
                t_loss += self.criterions[0](outputs[0][i],targets)
            t_loss = t_loss / len(outputs[0])
        else:
            t_loss = self.criterions[0](outputs[0],targets)
    ...
    def train(self, epoch, train_loader, optimizer, print_freq = 10,logger =None):
        ...
        for i, inputs in enumerate(train_loader):
            ...
            total_loss = t_loss +c_loss
            losses_meter.update(total_loss.item(), inputs[0].size(0))
            loss_t_meter.update(t_loss.item(),inputs[0].size(0))
            loss_c_meter.update(c_loss.item(),inputs[0].size(0))
            precisions.update(acc, 1)
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
...
class FinedTrainer2(object):
    ...
    def train(self, epoch, train_loader, optimizer, print_freq=10,logger =None, iter_n =0):
        self.model.train()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        losses = AverageMeter()
        ...
        for i, inputs in enumerate(train_loader):
            ...
            loss = self._forward(inputs, pids, epoch)
            ...
            if (i + 1) % print_freq == 0:
                logger.info('Iteration: {}\t'
                      ...
                      'Data {:.3f} ({:.3f})\t'
                      'Loss {:.3f} ({:.3f})\t'
                      .format(iter_n,epoch, i + 1, len(train_loader),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg,
                              losses.val, losses.avg,
                              ))
    ...
    def _forward(self, inputs, pids, epoch):
        loss = 0.0
        outputs = self.model(*inputs)
        loss_global = self.criterions[1](outputs[1], pids[0])
        loss = loss_global
        # outputs_split = [outputs[0][0], torch.cat(outputs[0][1:], dim=1)]
        if isinstance(outputs[0], list):
            for i, output_p in enumerate(outputs[0]):
                loss_tri = self.criterions[0](output_p, pids[i])
                loss += loss_tri
        else:
            loss_tri = self.criterions[0](outputs[0], pids[0])
            loss += loss_tri
