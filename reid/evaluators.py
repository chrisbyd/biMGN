...
from .evaluation_metrics.eval_metrics import evaluate
...
def evaluate_all(distmat, query=None, gallery=None,
                 ...
                 cmc_topk=(1, 5, 10),dataset_name = 'VeRi',logger =None):
    ...
    # Compute mean AP
    query_ids, gallery_ids,query_cams,gallery_cams = np.asarray(query_ids), np.asarray(gallery_ids), np.asarray(query_cams), np.asarray(gallery_cams)
    cmc, mAp = evaluate(distmat,query_ids, gallery_ids, query_cams,gallery_cams, dataset_name=dataset_name)
    logger.info("The map is {:.4f}, top-1,5,10,15,20 is {:.4f},{:.4f},{:.4f},{:.4f},{:.4f}".format(mAp,cmc[0],cmc[4],cmc[9],cmc[14],cmc[19]))
    return cmc, mAp
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None, dataset_name = 'VeRi', logger =None):
        features, _ = extract_features(self.model, data_loader, print_freq=self.print_freq)
        distmat = pairwise_distance(features, query, gallery, metric=metric)
        return evaluate_all(distmat, query=query, gallery=gallery, dataset_name = dataset_name, logger = logger)
